...
class AstronomyComputationService:
    ...
    def compute_astronomy_daily(self, location_id: int, lat: float, lon: float, target_date: date) -> dict[str, Any]:
        ...
        try:
            ...
            sunrise_utc = sun_data['sunrise'].replace(tzinfo=UTC)
            sunset_utc = sun_data['sunset'].replace(tzinfo=UTC)
            civil_twilight_start = sun_data['dawn'].replace(tzinfo=UTC)
            civil_twilight_end = sun_data['dusk'].replace(tzinfo=UTC)
            ...
            record = {
                ...
                "date": datetime.combine(target_date, datetime.min.time()).replace(tzinfo=UTC),
